...
def predict_2D_coordinates(model):
    ...
    for i in range(100):
        ...
        img = cv2.resize(cv2.imread(directory_path +"000"+str(100+i)+".is.png"),(512,512))
        predImg[0] = cv2.resize(img,(512,512))
        
        parser = Parser(directory_path + "000"+str(100+i)+".json")
        #-------------------
        predictedData = model.predict(predImg)
#        predictedData[predictedData < 0] = 0 #ka was das macht
        
        imgLabel = parser.get_2D_data()
        
        print("myData: ", imgLabel)
        print("myPredictedData: ", predictedData)
...
def predict_3D_coordinates(model):
    ...
    for i in range(2):
        predImg = np.zeros(shape=(1,512,512,3))
        img = cv2.resize(cv2.imread(directory_path + "000"+str(800+i)+".is.png"),(512,512))
        predImg[0] = cv2.resize(img,(512,512))
        
        parser = Parser(directory_path + "000"+str(800+i)+".json")
        #-------------------
        predictedData = model.predict(predImg)
...
def load_preTrained_model(modelPath):
...
show_hist(hist_2d)
modelTrainedTillStopp = model_2d
model_2d = load_preTrained_model("./model_"+dateTime)
#model_2d = load_model("./model_2020_01_30_15_17_03_epoch(10)-val_loss(0.0255)-loss(0.0182).hdf5")
